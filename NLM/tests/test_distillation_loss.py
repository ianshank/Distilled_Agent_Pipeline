...
class TestDistillationLoss:
    ...
    def test_pure_task_loss(self, sample_logits):
        ...
        loss_dict = compute_distillation_loss(
            student_logits, teacher_logits, labels, alpha=0.0, temperature=2.0
        )
    ...
    def test_pure_distillation_loss(self, sample_logits):
        ...
        loss_dict = compute_distillation_loss(
            student_logits, teacher_logits, labels, alpha=1.0, temperature=2.0
        )
    ...
    def test_mixed_loss(self, sample_logits):
        ...
        loss_dict = compute_distillation_loss(
            student_logits, teacher_logits, labels, alpha=0.5, temperature=2.0
        )
    ...
    def test_temperature_scaling(self, sample_logits):
        ...
        loss_low_temp = compute_distillation_loss(
            student_logits, teacher_logits, labels, alpha=1.0, temperature=1.0
        )

        loss_high_temp = compute_distillation_loss(
            student_logits, teacher_logits, labels, alpha=1.0, temperature=3.0
        )
    ...
    def test_ignore_index_handling(self):
        ...
        loss_dict = compute_distillation_loss(
            student_logits, teacher_logits, labels, alpha=0.0, temperature=2.0  # Pure task loss
        )
