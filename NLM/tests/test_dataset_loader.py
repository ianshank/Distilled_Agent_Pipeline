...
class TestConvertJsonlFormat:
    ...
    def test_basic_conversion(self, sample_jsonl_prompt_completion, temp_dir):
        ...
        count = convert_jsonl_format(str(sample_jsonl_prompt_completion), str(output_file))
    ...
    def test_missing_input_file(self, temp_dir):
        ...
        with pytest.raises(FileNotFoundError):
            convert_jsonl_format("nonexistent.jsonl", str(temp_dir / "output.jsonl"))
...
class TestLoadDistillationDataset:
    ...
    def test_load_text_format(self, sample_jsonl_text):
        ...
        dataset = load_distillation_dataset(
            train_file=str(sample_jsonl_text), tokenizer=tokenizer, max_length=64
        )
    ...
    def test_load_prompt_completion_format(self, sample_jsonl_prompt_completion):
        ...
        dataset = load_distillation_dataset(
            train_file=str(sample_jsonl_prompt_completion), tokenizer=tokenizer, max_length=64
        )
    ...
    def test_empty_dataset_error(self, temp_dir):
        ...
        with pytest.raises(ValueError, match="empty"):
            load_distillation_dataset(train_file=str(empty_file), tokenizer=tokenizer)
    ...
    def test_tokenization_length_limit(self, sample_jsonl_text):
        ...
        dataset = load_distillation_dataset(
            train_file=str(sample_jsonl_text), tokenizer=tokenizer, max_length=32  # Short length
        )
